...
2. The system will automatically assign a unique queue number to each customer. This number should be sequential, starting from 1.
3. There will be an option to call the next customer for service. The system must prioritize the priority queue at all times. If the priority queue is not empty, the next customer called will always be from that queue. Only when the priority queue is empty will the system call a customer from the regular queue.
